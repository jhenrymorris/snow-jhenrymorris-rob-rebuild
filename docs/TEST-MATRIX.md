...
| TM-101 | ROB two-signer sequence | Employee signature, separate supervisor approval, and supervisor signature execute in order with authoritative identity/timestamps | PDI/agency runtime | PARTIAL PASS — employee and supervisor signatures persisted in order; separate supervisor approval evidence remains unproven |
...
| TM-103 | Authoritative signed PDF | Exactly one immutable/auditable final PDF is associated with the Authorization Form and historical PDFs are retained | PDI/agency runtime/security | PARTIAL PASS — one system PDF is associated to synthetic native case HRC0001026 and completed tasks are read-only; Authorization Form placement, replacement audit, and independent history remain open |
...
| TM-107 | Native supervisor refusal | Supervisor refusal/denial prevents successful completion and final PDF | PDI runtime | NOT PROVEN — refusal control visible, but second execution could not be launched after Preview opened a blank child tab |
| TM-108 | Native signed-version retention | A later completed signed artifact does not overwrite the prior artifact or signer evidence | PDI runtime | NOT PROVEN — second completed signed version was not created |
