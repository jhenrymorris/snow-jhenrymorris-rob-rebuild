...
| TM-102 | Signed Form 1768 fidelity | April 2026 policy, approved fields, distinct IPA/WPC extensions, printed Date = Supervisor Signature Date / Final Authorization Date, and separate Electronic Authorization Metadata section render exactly | Policy plus PDI/agency runtime | BLOCKED — clean PDF `668b256cc33a0f1068a35f2b2b0131f6` rendered the approved structure/extensions, but `${Date}` is preparation-time data rather than the persisted supervisor signature date and actual signature timestamps cannot be rendered before signing. |
| TM-103 | Authoritative signed PDF | Exactly one immutable/auditable final PDF is associated with the Authorization Form and historical PDFs are retained | PDI/agency runtime/security | PARTIAL PASS — two independent native-case PDFs and signer histories are retained; completed tasks are read-only. Production Authorization Form-only placement/security remains unimplemented. |
