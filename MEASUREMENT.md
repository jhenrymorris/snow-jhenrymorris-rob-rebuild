...
- Shape: supported ServiceNow Studio application creation followed by a
  source-only scope migration on isolated branch
  `codex/dev437-rebuild-identity`.
- New identity: application `HR Access ROB Authorization Rebuild`, scope
  `x_2166123_hr_acc_0`, sys_id `6d508ed7833643104f5193a6feaad372`,
...
- Bootstrap cleanup: Studio-generated `.admin` and `.user` roles removed
  through supported Role forms; post-cleanup scoped-role count 0 before Fluent
  installation.
...
- Validation: M2 19/19, R1 9/9, Wave 2 security 22/22, deployment
  ...
  --check` PASS.
- Deployment: not executed. Git review, separate remote creation, IDE Sync,
  and explicit Build and Install authorization remain required.
