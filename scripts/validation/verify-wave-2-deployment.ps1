...
$requestedVariables = Invoke-ReadOnlyQuery `
    ...
    -Query 'name=x_2166123_rob_auth_requested_items' `
...
$caseFields = @($scopeDictionaries | Where-Object {
    ...
    (Get-RawValue $_ 'element').StartsWith('x_2166123_rob_auth_')
})
...
$caseAcls = @($scopeAcls | Where-Object {
    ...
    $aclName.StartsWith('sn_hr_core_case_payroll.x_2166123_rob_auth_') -or
    $aclName.StartsWith('sn_hr_core_case_workforce_admin.x_2166123_rob_auth_')
})
...
$taskAcls = Invoke-ReadOnlyQuery `
    -Table 'sys_security_acl' `
    -Query 'name=sn_hr_core_task.x_2166123_rob_auth_rob_task_type' `
    -Fields 'sys_id,name,operation,active,admin_overrides,condition,decision_type,script'
