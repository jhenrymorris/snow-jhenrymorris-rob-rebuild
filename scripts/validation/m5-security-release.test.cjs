...
test('scheduled job is daily, configuration-driven, and contains no external provisioning', () => {
    ...
    assert.match(runtime, /GlideDateTime\.subtract/)
    assert.doesNotMatch(runtime, /gs\.dateDiff/)
    assert.match(runtime, /rob_persist_authorization_lifecycle_native/)
    assert.match(runtime, /rob_lapse_authorized_access_detail_native/)
    assert.match(runtime, /\.inForeground\(\)/)
    assert.doesNotMatch(runtime, /authorization\.update\(\)|detail\.update\(\)/)
...
test('governed record ACLs deny broad repository and edit access', () => {
    ...
    assert.doesNotMatch(source, /rob_operations_manager.*hasRole/)
