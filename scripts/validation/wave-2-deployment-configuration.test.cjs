...
test('employee-facing dependencies remain portal-safe and self-only', () => {
    ...
    assert.match(requesterSecurity, /requesterId !== authenticatedUserId/)
    assert.match(requesterSecurity, /requesterId !== openedForId/)
    assert.match(requesterSecurity, /requesterId !== subjectPersonId/)
})
