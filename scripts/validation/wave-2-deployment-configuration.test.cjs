...
test('both requested-items variables are active mandatory list collectors', () => {
    for (const source of [staffingVariables, analyticsVariables]) {
        ...
        assert.match(
            source,
            /x_2166123_rob_auth_requested_items:\s*ListCollectorVariable/
        )
        assert.match(
            source,
            /listTable:\s*x_2166123_rob_auth_rob_access\.name/
        )
...
test('six starter access items and exact categories are source controlled', () => {
    assert.equal(
        (accessItems.match(/table:\s*'x_2166123_rob_auth_rob_access'/g) || [])
            .length,
        6
    )
    assert.deepEqual(
...
test('all employee-facing case payload fields are SDK managed', () => {
    ...
    assert.match(
        caseFields,
        /x_2166123_rob_auth_employment_type:\s*ChoiceColumn/
    )
    assert.match(
        caseFields,
        /x_2166123_rob_auth_access_end_date:\s*DateColumn/
    )
    assert.match(
        caseFields,
        /x_2166123_rob_auth_operations_manager:\s*ReferenceColumn/
    )
    assert.match(
        caseFields,
        /x_2166123_rob_auth_operations_manager:[\s\S]*?referenceTable:\s*'sys_user'[\s\S]*?referenceQual:\s*'active=true'/
    )
})
...
test('employee-facing dependencies remain portal-safe and self-only', () => {
    ...
    assert.match(
        commonVariables,
        /x_2166123_rob_auth_business_justification:\s*MultiLineTextVariable/
    )
    assert.match(commonVariables, /question:\s*'Business Justification'/)
    assert.match(
        analyticsVariables,
        /x_2166123_rob_auth_operations_manager:\s*ReferenceVariable/
    )
    assert.match(
        commonPolicies,
    ...
    assert.doesNotMatch(
        requesterSecurity,
        /setValue\(['"]x_2166123_rob_auth_(?:position_title|organization_snapshot|supervisor_snapshot)/
    )
    assert.match(
        commonVariables,
        /x_2166123_rob_auth_selected_supervisor:\s*ReferenceVariable/
    )
    assert.match(
        commonVariables,
        /x_2166123_rob_auth_organization_fallback:\s*ReferenceVariable/
    )
})
