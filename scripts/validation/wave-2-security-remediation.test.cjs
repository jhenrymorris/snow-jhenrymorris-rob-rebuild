...
const names = {
    items: 'x_2166123_rob_auth_requested_items',
    title: 'x_2166123_rob_auth_position_title',
    organization: 'x_2166123_rob_auth_organization_snapshot',
    supervisor: 'x_2166123_rob_auth_supervisor_snapshot',
    exception: 'x_2166123_rob_auth_exception_review_required',
    reason: 'x_2166123_rob_auth_exception_reason',
    blocked: 'x_2166123_rob_auth_authorization_processing_blocked',
    employeeGate: 'x_2166123_rob_auth_requires_employee_signature',
    supervisorGate: 'x_2166123_rob_auth_requires_supervisor_signature',
    fulfillmentGate: 'x_2166123_rob_auth_fulfillment_gate_complete',
    correctionRequested: 'x_2166123_rob_auth_snapshot_correction_requested',
    correctionReason: 'x_2166123_rob_auth_snapshot_correction_reason',
    priorTitle: 'x_2166123_rob_auth_prior_position_title',
    priorOrganization: 'x_2166123_rob_auth_prior_organization_snapshot',
    priorSupervisor: 'x_2166123_rob_auth_prior_supervisor_snapshot',
    correctedBy: 'x_2166123_rob_auth_snapshot_corrected_by',
    correctedAt: 'x_2166123_rob_auth_snapshot_corrected_at',
}
...
const preservedMappings = [
    ...
    'short_description',
    'rich_description',
    'x_2166123_rob_auth_employment_type',
    'x_2166123_rob_auth_access_end_date',
    names.items,
    'x_2166123_rob_auth_operations_manager',
    'assignment_group',
]
...
function glideRecordFactory(state) {
    return function GlideRecord(table) {
        ...
        this.query = () => {
            if (table === 'x_2166123_rob_auth_rob_access') {
            ...
            } else if (table === 'x_2166123_rob_auth_rob_config') {
...
function baseCase(overrides = {}) {
    return {
        ...
        x_2166123_rob_auth_employment_type: 'federal_employee',
        x_2166123_rob_auth_access_end_date: '2027-09-30',
...
test('only the four approved employment types are accepted and IPA is not forced to have an end date', () => {
    for (const employmentType of [
        ...
        const result = run(
            snapshotScript,
            baseCase({
                x_2166123_rob_auth_employment_type: employmentType,
                x_2166123_rob_auth_access_end_date:
    ...
    const invalid = run(
        snapshotScript,
        baseCase({ x_2166123_rob_auth_employment_type: 'other_time_limited' })
    )
...
test('WPC requires one active Operations Manager and remains Analytics-only', () => {
    ...
    const valid = run(snapshotScript, {
        ...baseWpc,
        x_2166123_rob_auth_operations_manager: 'operations_manager',
    })
...
test('supervisor exception task creation is idempotent and configuration-routed', () => {
    ...
    assert.equal(
        first.state.inserts[0].x_2166123_rob_auth_rob_task_type,
        'exception_review'
    )
...
test('snapshot reads require a coherent self-submission and task type is immutable', () => {
    ...
    assert.match(
        aclSource,
        /\$id: Now\.ID\['hr-task-rob-task-type-write'\][\s\S]*?field: 'x_2166123_rob_auth_rob_task_type'[\s\S]*?script: 'return false;'/
    )
})
